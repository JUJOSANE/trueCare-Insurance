...
function SchedulePage() {
  const [selectedAdvisor, setSelectedAdvisor] = useState("rosario");
  const { t } = useLanguage();

  const selectedAdvisorData = advisors.find(
    (advisor) => advisor.translationKey === selectedAdvisor
  );
    ...
    <>
      ...
      <section className="bg-slate-50 px-6 py-20 md:px-12 lg:px-24">
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mb-8 flex flex-col justify-center gap-4 lg:hidden">
            {advisors.map((advisor) => (
              <button
                key={advisor.translationKey}
                onClick={() => setSelectedAdvisor(advisor.translationKey)}
                className={`rounded-xl px-6 py-4 font-semibold transition ${
                  selectedAdvisor === advisor.translationKey
          ...
          <div className="lg:hidden">
            <AdvisorCard advisor={selectedAdvisorData} t={t} />
          </div>
